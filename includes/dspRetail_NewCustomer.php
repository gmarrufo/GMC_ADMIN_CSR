    ...
    if (isset($_POST['cmdNewCustomer']))
    {
        ...
        $intTotalRows = mssql_num_rows($rsEmailExists);
         ...
         if($intTotalRows > 0)
         {
             ...
             header("Location: https://secure.revitalash.com/retail/begin_cart.php?eh=101");
             // header("Location: http://localhost/retail/begin_cart.php?eh=101");
         }
         else
         {
	...
	function validate_new(thisform)
	{
		with (thisform)
		{
			if (validate_required(FirstName,"You must enter your first name")==false)
             ...
             // GMC - 05/24/10 - To validate if customer hits new address
             var radioObj = document.form.AddressAction;
             ...
             if(rad_val == "CreateNew")
             {
			     if (validate_required(ShipToAttn,"You must enter ship to attention")==false)
		  	     {return false;}
			     else if (validate_required(ShipToAddress1,"You must enter ship to address")==false)
		  	     {return false;}
			     else if (validate_required(ShipToCity,"You must enter ship to city")==false)
		  	     {return false;}
			     else if (validate_required(ShipToPostalCode,"You must enter ship to postal code")==false)
		  	     {return false;}
			     else if (validate_required(ShipToCountryCode,"You must enter ship to country")==false)
		  	     {return false;}
             }
		}
	}
...
<div id="wrapper">
<!--<div class="consumer_header">New Account<br>Set Up<span style="margin-left:540px;">Revitalash&reg;</span></div>-->
<div class="consumer_header">New Account Set Up</div>

<form action="../includes/dspRetail_NewCustomer.php" method="post" name="form" onSubmit="return validate_new(this);">
...
<td>
    ...
    <tr>
        ...
        <td><input type="text" name="EMailAddress" size="40" value="" /><br><font color="red"><b>Your Email is used to send you Shipping Information</b></font></td>
    </tr>
...
<td>
    ...
    <tr>
        <th width="100">&nbsp;</th>
        <td width="*" style="font-size:10px; font-weight:bold;">
        <input name="AddressAction" type="radio" value="SameAsBilling" checked="checked" /> Same As Billing
        <input name="AddressAction" type="radio" value="CreateNew" /> New Address</td>
    </tr>
    ...
    <tr>
        <th>Country Code:</th>
        <td><select name="ShipToCountryCode" size="1"  onChange="getStateProvinceShip(this.value)">
        <?php echo $selectCountries; ?>
        </select></td>
    </tr>
...
<tr>
	<td colspan="2" align="center"><input type="submit" name="cmdNewCustomer" value="Continue" class="formSubmit" /></td>
</tr>
